...
def main():
    ...
    assert_true(len(rows) == 8, "لابی حالت ۴نفره باید ۸ ردیف باشه (شاملِ ردیفِ سطحِ دشواری)")
    ...
    g.cycle_team_size()  # 8
    rows = build_lobby_rows(g)
    # انتظار: هدر + رنگ + برچسب‌مامور + ۳ردیف‌مامور + برچسب‌جاسوس + ۱ردیف‌جاسوس + سطحِ دشواری + فوتر = ۱۰
    assert_true(len(rows) == 10, f"لابی حالت ۸نفره باید ۱۰ ردیف باشه، شد {len(rows)}")
